...
TEST_F(ExportJsonTest, V8CpuProfileFromTables) {
  ...
  PerfettoSqlEngine* engine = impl->engine();
  ...
  storage->mutable_cpu_profile_stack_sample_table()->Insert(
      {kSampleTs0, cs_foo, utid, 0});
  storage->mutable_cpu_profile_stack_sample_table()->Insert(
      {kSampleTs1, cs_foo, utid, 0});
  ...
  EXPECT_FALSE(pdata.HasMember("source"));
  ...
  EXPECT_FALSE(cdata.HasMember("source"));
  ...
  EXPECT_FALSE(edata.HasMember("source"));
