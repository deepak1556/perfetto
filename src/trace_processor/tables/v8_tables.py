    ...
    tabledoc=TableDoc(
        ...
        columns={
            ...
            'end_time_us':
                ...
                ' profiler.',
            'start_thread_ts':
                'Thread time at session start (NULL if V8 did not report it).',
            'end_thread_ts':
...
V8_CPU_PROFILE_CHUNK = Table(
    ...
    tabledoc=TableDoc(
        doc="""
          One row per V8CpuProfileChunk packet (data chunk only — session_start
          and session_end packets are recorded on v8_cpu_profile_session). Used
...
V8_CPU_PROFILE_NODE = Table(
    ...
    tabledoc=TableDoc(
        doc="""
          ...
          tables. The `(v8_cpu_profile_session_id, node_id)` pair matches the
          devtools `cpuProfile.nodes[].id`; the `callsite_id` links to the
          shared callsite/frame tables.
        """,
        group='v8',
        columns={
...
V8_CPU_PROFILE_TRACE_ID = Table(
    ...
    tabledoc=TableDoc(
        doc="""
          ...
          correlation. Mirrors the `cpuProfile.trace_ids` map in the legacy
          devtools JSON `ProfileChunk` event.
        """,
        group='v8',
        columns={
            'v8_cpu_profile_session_id': 'Owning profiling session.',
            'trace_id': 'Trace event id to correlate with.',
            'node_id': 'V8 node id this trace event maps to.',
        },
    ),
)
