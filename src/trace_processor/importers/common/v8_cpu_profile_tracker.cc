...
void V8CpuProfileTracker::Parse(int64_t ts, V8CpuProfileEvent event) {
  base::Status status =
...
base::Status V8CpuProfileTracker::AddSample(int64_t ts,
                                            ...
                                            uint32_t tid,
                                            uint32_t raw_callsite_id) {
  return AddSampleWithMeta(ts, session_id, pid, tid, raw_callsite_id,
...
void V8CpuProfileTracker::AddNodeMetadata(
  ...
  auto* state = state_by_session_and_pid_.Find(std::make_pair(session_id, pid));
  if (!state || !state->session_row_id || !state->current_chunk_row_id) {
    return;
  }
...
base::Status V8CpuProfileTracker::AddSampleWithMeta(
  ...
  if (state->session_row_id && state->current_chunk_row_id) {
