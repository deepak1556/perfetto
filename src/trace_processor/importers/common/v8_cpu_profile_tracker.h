...
// Stores interned callsites for given pid for v8 samples.
class V8CpuProfileTracker
    : public TraceSorter::Sink<V8CpuProfileEvent, V8CpuProfileTracker> {
 public:
  explicit V8CpuProfileTracker(TraceProcessorContext*);
  ~V8CpuProfileTracker() override;

  void Parse(int64_t ts, V8CpuProfileEvent);
 ...
 private:
  struct State {
    ...
    // Set when BeginSession is called (i.e. for the V8CpuProfileChunk proto
    // import path). Unused by the legacy JSON path.
    std::optional<tables::V8CpuProfileSessionTable::Id> session_row_id;
    // Most recent BeginChunk for the new proto path, used to tag nodes and
    // samples with the chunk that introduced them.
